...
async function main() {
  ...
  for (const module of modules) {
    ...
    if (finalTopics.length > 0) {
      ...
      // Perform the update for JUST THIS ONE MODULE in several steps to avoid ANY timeout
      try {
          ...
          console.log(`  -> Successfully saved "${module.title}"`);
      } catch (err) {
          console.error(`  -> Failed to save "${module.title}":`, err);
      }
    }
  }

  console.log("\nSafe Premium Restructure complete!");
}
